...
FYI, this is the representation of the internal structure of a typical JBC T245 Tip. I wonder how they made the tip; it truly is an engineering marvel. Basically, it has two metal tubes. The inner one is the **Common (C3)**. The next layer is covered by an insulator, and the layer after that is the second tube **Heater (C2)**. At the top of the tip, it is designed with a spiral heater configuration, as you can see in the picture above.

The thermocouple placement is very interesting. They didn't provide separate pins for the heating element and the thermocouple. One of the thermocouple pins and the heating element pin are combined into one pin. For better understanding, I have provided the schematic configuration of the tip. I hope that combining these pins is easy for manufacturing. As you can see from the picture, the **Thermocouple (C1)** is also a part of the **Common (C3)**. The thermocouple is formed by intersecting both metals. I believe they used a different metal fused in the intersection of both metals. This is how they make the thermocouple. Isn't it interesting?

If you want to learn more about thermocouples, you can click [here](https://www.iqsdirectory.com/articles/thermocouple.html).

Also, from my investigation, I found that it's an N-type thermocouple, if I am not mistaken. For testing the thermocouple, I used boiling water as a reference temperature (100°C) and measured the voltage across the thermocouple. I obtained a reading of approximately 2.7mV. I used my **Mini Hotplate MHP30** to measure different temperatures and noted the voltage readings. I am not sure about the accuracy of my data, but I trust my findings. Based on the data, I concluded that the thermocouple is probably N-type. I used a thermocouple chart to check my readings.
...
### 3.6 Potentiometer
Initially, I considered using a rotary encoder to control the menu, but for the sake of simplicity, I opted to use a potentiometer for adjusting the set temperature. Additionally, I utilized a simple pushbutton for selecting the desired set temperature. This setup offers a straightforward and user-friendly interface.
